...
So, from the comments the professor made on our task 3, I am gathering that we could first split the data into X and Y, do all the preprocessing steps, then do the training, validation and test split?
...
n=nrow(xFull)
trainingDataIndex = createDataPartition(xFull$loan_status, p = .5, list = FALSE) %>% as.vector(.)
trainingData = xFull[trainingDataIndex, ]

validIndex = createDataPartition(xFull$loan_status[-trainingDataIndex], p = .5, list = FALSE) %>% as.vector(.)
validData = xFull[-trainingDataIndex, ][validIndex, ]
testingData = xFull[-trainingDataIndex, ][-validIndex, ]
...
Yvalid = select(validData, loan_status) %>% unlist() %>% as.factor()
...
Creating matrices for train and validation sets
```{r}
...
Probridge = predict(ridgeOut, XvalidMat, s = ridgeOut$lambda.min, type='response')
Yhatvalidridge = ifelse(Probridge > 0.5, 'Fully Paid', 'Default')

#Getting default confusion matrix
#Confusion Matrix
confusionMatrixR = table(Yhatvalidridge, Yvalid)
confusionMatrixR
...
Problasso = predict(lassoOut, XvalidMat, s = lassoOut$lambda.min, type='response')
Yhatvalidlasso = ifelse(Problasso > 0.5, 'Fully Paid', 'Default')

#Confusion Matrix
confusionMatrixL = table(Yhatvalidlasso, Yvalid)
confusionMatrixL
...
#Xvalid
Xvalidfull = cbind(YvalidRelevel,XvalidMat) %>% as.data.frame()
XvalidfullMat = select(Xvalidfull, -YvalidRelevel) %>% as.matrix()
dimnames(XvalidfullMat) = NULL
#Yvalid
Yvalidfull = select(Xvalidfull, YvalidRelevel) %>% unlist() %>% as.vector()
Yvalidfull = to_categorical(Yvalidfull)
...
my_model = model %>% fit(x=XfullMat, y=Yfull, epoch=20, batch_size=1, validation_data=list(Xvalid, Yvalid))


model %>% evaluate(Xvalid, Yvalid)

YhatvalidNN = model %>% predict(XvalidMat)
Yhatvalidlasso = ifelse(Problasso > 0.5, 'Fully Paid', 'Default')

#Confusion Matrix
confusionMatrixOut = confusionMatrix(reference = Yvalid, data = YhatvalidNN)
confusionMatrixOut$table
print(confusionMatrixOut$overall[1:2])
print(confusionMatrixOut$byClass[1:2])
...
YvalidFDA = as.character(YvalidRelevel)
YvalidFDA = sub("Fully ", "", YvalidFDA) %>% as.factor() %>% unlist()
...
YhatvalidFDA = predict(fdaOut, XvalidMat, type='raw')

#Confusion Matrix
confusionMatrixOut = confusionMatrix(reference = YvalidFDA, data = YhatvalidFDA)
confusionMatrixOut$table
print(confusionMatrixOut$overall[1:2])
print(confusionMatrixOut$byClass[1:2])
...
Yhatvalidsvm = predict(svmOut, XvalidMat)

#confusion matrix
confusionMatrixOut = confusionMatrix(reference = Yvalid, data = Yhatvalidsvm)
confusionMatrixOut$table
print(confusionMatrixOut$overall[1:2])
print(confusionMatrixOut$byClass[1:2])
...
knnOut   = train(x = XtrainMat, y = Ytrain,
                 ...
                 trControl = trainControl)
knnOut$bestTune
probKNN = predict(knnOut, XvalidMat, type='raw')

#confusion matrix
confusionMatrixOut = confusionMatrix(reference = Yvalid, data = YhatvalidKNN)
confusionMatrixOut$table
print(confusionMatrixOut$overall[1:2])
print(confusionMatrixOut$byClass[1:2])
```
...
YhatvalidTree = predict(rpartOut, XvalidMat)

#confusion matrix
confusionMatrixOut = confusionMatrix(reference = Yvalid, data = YhatvalidTree)
confusionMatrixOut$table
print(confusionMatrixOut$overall[1:2])
print(confusionMatrixOut$byClass[1:2])
...
YhatvalidRF = predict(rfOut, XvalidMat)
#confusion matrix
confusionMatrixOut = confusionMatrix(reference = Yvalid, data = YhatvalidRF)
confusionMatrixOut$table
print(confusionMatrixOut$overall[1:2])
print(confusionMatrixOut$byClass[1:2])
...
Yhatvalidboost = predict(boostOut, XvalidMat)
#confusion matrix
confusionMatrixOut = confusionMatrix(reference = Yvalid, data = Yhatvalidboost)
confusionMatrixOut$table
print(confusionMatrixOut$overall[1:2])
print(confusionMatrixOut$byClass[1:2])
```
